...
from app.schemas import ProductModel, ProductUpdateModel
from app.jwt import get_current_active_user
from typing import List
from app.models import User
from crud.product import get_product_by_id, get_all_products, create_product, update_product, delete_product_by_id
'''
...
def create_product_view(product_create: ProductModel, current_user: User = Depends(get_current_active_user)):
    product = create_product(user=current_user, **product_create.dict())
    return product

@router.put("/{product_id}", response_model=ProductModel)
def update_product_view(product_id: int, product_update: ProductUpdateModel, current_user: User = Depends(get_current_active_user)):
    product = get_product_by_id(product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
...
def delete_product_view(product_id: int, current_user: User = Depends(get_current_active_user)):
    product = get_product_by_id(product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    if product.user_id != current_user.id:
